...
crime.abbvs <- substr(snf$crime.suspected, 1, 3)
sorted.crimes.abbvs <- rev(sort(table(crime.abbvs)))
...
for(i in 1:6){
  print(rev(sort(table(crime.abbvs[snf$race == i]))))
}
...
time.of.stop <- substr(snf$time, 12, 13)
time.of.stop.sorted <- rev(sort(table(as.numeric(hour))))
...
#weight
good.weight <- snf[snf$weight > 40 & snf$weight < 400,]
